...
🎯 V4 COMPETITIVE WINDOW DEFINITIONS (250ms):
...
✅ 250ms WINDOWS: Precise boundary detection with focused temporal context
...
- 250ms FOCUSED WINDOWS: Precise boundary detection with optimal temporal context
...
def create_default_prosodic_features(seq_len=13):
    """
    Create default prosodic features when extraction fails or for dummy samples.
    
    Args:
        seq_len: Sequence length for temporal features (default ~13 for 250ms at 50Hz)
...
def clean_prosodic_features(prosodic_features, verbose=False, seq_len=13):
    """
    ...
    4. Providing fallback defaults for corrupted features
        ...
        seq_len: Expected sequence length for temporal features
    ...
    if prosodic_features is None:
        ...
        return create_default_prosodic_features(seq_len)
    ...
    def clean_temporal_feature(key, feature_data, target_len=seq_len):
    ...
    for key in simple_temporal_keys:
        if key in prosodic_features:
            ...
            cleaned[key] = np.zeros(seq_len, dtype=np.float32)
            ...
            if feature_type in prosodic_features:
                ...
                if isinstance(mfcc_sequences, (list, tuple)):
                    ...
                    while len(cleaned_sequences) < 6:
                        cleaned_sequences.append(np.zeros(seq_len, dtype=np.float32))
                    ...
                    cleaned[feature_type] = [np.zeros(seq_len, dtype=np.float32) for _ in range(6)]
...
class CompetitiveWindowPreprocessor:
    ...
    def __init__(self, data, processor, window_duration=0.25, sample_rate=16000, 
        ...
        Args:
            window_duration: Window duration in seconds (default: 0.25s for precise detection)
        ...
        if verbose:
            ...
            print(f"   💡 250ms windows: Boundary in last 20ms = {(boundary_tolerance/window_duration)*100:.1f}% (strong signal)!")
    ...
    def extract_prosodic_features(self, audio):
        ...
        try:
            ...
            if target_length <= 0:
                target_length = 13  # Default fallback for 250ms windows (~12.5 frames)
            ...
            prosodic_features = clean_prosodic_features(prosodic_features, seq_len=target_length)
            ...
            return create_default_prosodic_features(target_length if 'target_length' in locals() else 13)
    
    def _prosodic_dict_to_tensor(self, prosodic_dict, seq_len=13):
...
class PreprocessedWindowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            return {
                'input_values': torch.zeros(4000, dtype=torch.float32),  # 250ms at 16kHz
                'label': torch.tensor(0.0, dtype=torch.float32),
                'file_id': f'dummy_{idx}',
                'prosodic_features': torch.zeros(28, 13, dtype=torch.float32),  # TENSOR, not dict!
                ...
                if targets[i] == 0 and isinstance(meta, dict):
                    ...
                    if 'boundary_pos_in_window' in meta:
                        ...
                        if boundary_pos is not None:
                            ...
                            window_samples = 4000  # 250ms * 16kHz
...
def predict_boundaries_competitive(model, audio, processor, device, 
                                   window_duration=0.25, stride=0.01, 
    ...
    Args:
        ...
        window_duration: Window duration (250ms for precise boundary detection)
        ...
        for start_pos in range(0, len(audio) - window_samples + 1, stride_samples):
            ...
            if prosodic_extractor is not None:
                try:
                    ...
                    if seq_len <= 0:
                        seq_len = 13  # Default fallback for 250ms windows
...
def test_prosodic_feature_fix():
    ...
    try:
        # Create test audio
        test_audio = np.random.randn(4000).astype(np.float32)  # 250ms at 16kHz
        ...
        # Convert to tensor
        prosodic_tensor = prosodic_extractor._prosodic_dict_to_tensor(prosodic_dict, 13)
        
        print(f"✅ Prosodic feature extraction successful!")
        print(f"   Tensor shape: {prosodic_tensor.shape}")
        print(f"   Expected: [28, 13]")
...
def custom_collate_fn(batch):
    ...
    if len(valid_samples) == 0:
        ...
        dummy_sample = {
            'input_values': torch.zeros(4000),  # 250ms at 16kHz
            'label': torch.tensor(0.0),
            'file_id': 'dummy_batch',
            'prosodic_features': torch.zeros(28, 13, dtype=torch.float32),  # TENSOR!
            'metadata': {'boundary_pos': None, 'window_start': 0}
        }
    ...
    try:
        ...
        for sample in valid_samples:
            ...
            if prosodic is None or not isinstance(prosodic, torch.Tensor):
                ...
                prosodic = torch.zeros(28, 13, dtype=torch.float32)  # 250ms windows
        ...
        return {
            'input_values': torch.zeros(batch_size, 4000),  # 250ms at 16kHz
            'label': torch.zeros(batch_size),
            'file_id': [f'error_dummy_{i}' for i in range(batch_size)],
            'prosodic_features': torch.zeros(batch_size, 28, 13, dtype=torch.float32),  # TENSOR!
...
def main():
    ...
    config = {
        ...
        # COMPETITIVE WINDOW GENERATION - 250ms WINDOWS
        'window_duration': 0.25,             # COMPETITIVE: 250ms windows for precise boundary detection
        ...
        # COMPETITIVE PREPROCESSING DIRECTORIES - 250ms
        'train_preprocessed_dir': './preprocessed_windows_train_250ms_competitive',  # COMPETITIVE: ±20ms tolerance + prosodic
        'test_preprocessed_dir': './preprocessed_windows_test_250ms_competitive',    # COMPETITIVE: ±20ms tolerance + prosodic
    ...
    print(f"   📏 250ms FOCUSED WINDOWS: Precise boundary detection with optimal temporal context")
    ...
    print("📏 NEW: 250ms FOCUSED WINDOWS for precise boundary detection!")
