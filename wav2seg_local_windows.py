...
class LocalBoundaryClassifier(nn.Module):
    """
    Simple CNN classifier for local boundary detection.
    
    Takes 0.5s windows and predicts binary boundary probability.
    Much simpler than sequence-to-sequence models.
    """
    
    def __init__(self, wav2vec2_model_name="facebook/wav2vec2-base", freeze_wav2vec2=True, 
                 hidden_dim=256, dropout_rate=0.3, label_smoothing=0.0):
        """
        Initialize the local boundary classifier.
        ...
        print(f"      🏗️ Building classification head...")
        # POSITION-AWARE Classification head - focuses on window END where boundary is!
        self.classifier = nn.Sequential(
            # Temporal convolutions to capture local boundary transition patterns
            nn.Conv1d(wav2vec2_hidden_size, 256, kernel_size=7, padding=3),  # Capture transitions
            nn.ReLU(),
            nn.BatchNorm1d(256),
            nn.Dropout(dropout_rate),
            
            nn.Conv1d(256, 128, kernel_size=5, padding=2),  # Refine boundary patterns
            nn.ReLU(), 
            nn.BatchNorm1d(128),
            nn.Dropout(dropout_rate),
            
            nn.Conv1d(128, 64, kernel_size=3, padding=1),   # Final temporal features
            nn.ReLU(),
            nn.BatchNorm1d(64),
        )
        ...
        # CRITICAL: Proper weight initialization to prevent NaN
        self._initialize_weights()
        
        print(f"      ✅ Classification head built successfully")
        print(f"      🔧 Weights properly initialized to prevent NaN")
        
    def _initialize_weights(self):
        """Initialize classifier weights properly to prevent NaN/inf issues."""
        for module in self.classifier.modules():
            if isinstance(module, nn.Linear):
                # MUCH MORE CONSERVATIVE initialization to prevent NaN
                nn.init.xavier_normal_(module.weight, gain=0.1)  # Very small gain
                if module.bias is not None:
                    nn.init.constant_(module.bias, 0.0)  # Zero bias
            elif isinstance(module, nn.Conv1d):
                # Conservative conv initialization
                nn.init.kaiming_normal_(module.weight, mode='fan_out', nonlinearity='relu', a=0.01)
                if module.bias is not None:
                    nn.init.constant_(module.bias, 0.0)
        
        # Initialize boundary detector with even smaller weights
        for module in self.boundary_detector.modules():
            if isinstance(module, nn.Linear):
    ...
    def forward(self, input_values):
        """
        Forward pass: audio window -> boundary logits.
        POSITION-AWARE: Focuses on the END of the window where boundary should be.
        
        Args:
            input_values: Audio input [batch_size, seq_len]
            
        Returns:
            torch.Tensor: Boundary logits [batch_size] (NOT probabilities - use with BCEWithLogitsLoss)
        """
        ...
        hidden_states = hidden_states.transpose(1, 2)  # [batch, 768, seq_len]
        
        # Extract temporal features with CNNs
        temporal_features = self.classifier(hidden_states)  # [batch, 64, seq_len]
        
        # Get per-timestep boundary scores
        temporal_features = temporal_features.transpose(1, 2)  # [batch, seq_len, 64]
        timestep_scores = self.boundary_detector(temporal_features)  # [batch, seq_len, 1]
...
def train_local_classifier(model, train_dataloader, val_dataloader, device, num_epochs=10, 
...
def main():
    ...
    # FIXED Configuration - Generate positive windows for ALL boundaries, balance with negatives
    config = {
        # STABLE TRAINING - PREVENT NaN
        'batch_size': 32,                    # REDUCED: Smaller batches for stability
        'num_epochs': 15,                    # More epochs with better regularization
        'learning_rate': 1e-5,               # INCREASED: Start with higher learning rate for better training
        'weight_decay': 1e-4,                # STRONGER weight decay for regularization
        
        # PROPER WINDOW GENERATION - ONE WINDOW PER BOUNDARY WITH BALANCED NEGATIVES
        'max_windows_per_file': None,        # REMOVED: No artificial limits
        'max_positive_per_file': None,       # REMOVED: Generate ONE window per boundary!
        'max_negative_per_file': 18,        # INCREASED: Allow up to 200 negatives for balance
        
        # IMPROVED NEGATIVE SAMPLING - Smaller exclusion zone, only for negatives
        'window_duration': 1.5,              # Keep 1.5s windows for context
        'boundary_tolerance': 0.06,          # Keep 60ms tolerance for positive windows
        'negative_exclusion_zone': 0.05,     # REDUCED: 50ms exclusion zone (was 150ms)
        'negative_sampling_ratio': 0.3,      # 30% of valid positions for reasonable count
        
        # STRONG REGULARIZATION TO PREVENT NaN
        'pos_weight': 2.0,                   # REMOVED bias to prevent instability
        'hidden_dim': 128,                    # Model capacity
        'dropout_rate': 0.4,                 # INCREASED: Strong dropout to prevent overfitting
        'label_smoothing': 0.1,             # REDUCED: Less aggressive smoothing
        
        # HARDWARE OPTIMIZATIONS
        'device': 'cuda' if torch.cuda.is_available() else 'cpu',
        'mixed_precision': False,            # DISABLED: Mixed precision can cause NaN
        'use_gradient_checkpointing': True,  # Memory optimization
        
        # EARLY STOPPING AND EFFICIENCY
        'early_stopping_patience': 6,       # More patience with lower LR
        'min_improvement': 0.001,           # Smaller improvement threshold
        
        # FIXED THRESHOLD APPROACH
        'threshold': 0.5,                    # FIXED: Use standard 0.5 threshold throughout
        
        # TWO-STAGE PREPROCESSING - New directory for FIXED approach
        'train_preprocessed_dir': './preprocessed_windows_train_fixed',  # NEW DIR for fixed approach
        'test_preprocessed_dir': './preprocessed_windows_test_fixed',    # NEW DIR for fixed approach
        'force_reprocess': True,             # FORCE reprocessing with fixed settings
        'sample_rate': 16000,
    }
...
persistent_workers=True)
    ...
    try:
        # Try with memory optimizations
        model = LocalBoundaryClassifier(
            freeze_wav2vec2=True,
            hidden_dim=config['hidden_dim'],        # Use config value
            dropout_rate=config['dropout_rate'],    # Use config value
            label_smoothing=config['label_smoothing']  # Use config value
        )
