"""
INTELLIGENT LOCAL WINDOW BOUNDARY DETECTION - V4 LEARNED ATTENTION WITH HARD EXAMPLE MINING
=============================================================================================

🧠 INTELLIGENT ARCHITECTURE: Learned attention for boundary detection with hard example mining

KEY IMPROVEMENTS:
✅ LEARNED ATTENTION: Model learns which parts of window are most informative
✅ HARD EXAMPLE MINING: Dynamically focuses training on difficult samples
✅ CUSTOM RANDOM SPLIT: Option to create custom train/test splits from complete TIMIT
...
🔥 HARD EXAMPLE MINING FEATURES:
✅ DYNAMIC DIFFICULTY SCORING: Tracks prediction confidence for each sample
✅ ADAPTIVE SAMPLING: Increases probability of hard examples in subsequent epochs
✅ LOSS-BASED WEIGHTING: Up-weights samples with high loss during training
✅ CONFIDENCE-BASED MINING: Focuses on samples with uncertain predictions (0.3-0.7 range)
✅ TEMPORAL PATTERN LEARNING: Maintains difficult samples across epochs for consistency

🎲 CUSTOM RANDOM SPLIT FEATURES:
✅ SPEAKER-INDEPENDENT SPLITS: Ensures no speaker overlap between train/test
✅ DIALECT-BALANCED: Maintains dialect region balance across splits
✅ CONFIGURABLE RATIOS: Customizable train/validation/test split ratios
✅ REPRODUCIBLE SPLITS: Seed-based splitting for consistent results
✅ FALLBACK TO OFFICIAL: Option to use official TIMIT splits as backup
...
PARADIGM: Local Binary Classification with Asymmetric Window Definitions + Hard Example Mining
...
- Hard example mining: Dynamically focuses on difficult samples
"""
...
class HardExampleMiner:
    """
    🔥 ADVANCED HARD EXAMPLE MINING SYSTEM
    
    Dynamically identifies and focuses training on difficult samples.
    Tracks sample difficulty across epochs and adaptively increases sampling probability.
    """
    
    def __init__(self, 
                 mining_method='confidence_based',  # 'confidence_based', 'loss_based', 'hybrid'
                 difficulty_threshold=0.7,           # Samples with confidence < threshold are "hard"
                 mining_start_epoch=3,              # Start mining after model has some basic learning
                 max_hard_ratio=0.4,                # Maximum fraction of batch that can be hard examples
                 update_frequency=1,                # Update difficulty scores every N epochs
                 confidence_range=(0.3, 0.7),       # Confidence range for "uncertain" samples
                 loss_percentile=75,                # Top N% loss samples are considered hard
                 momentum=0.9):                     # Momentum for smoothing difficulty scores
        """
        Initialize the hard example mining system.
        
        Args:
            mining_method: Strategy for identifying hard examples
            difficulty_threshold: Confidence threshold for hard examples
            mining_start_epoch: Epoch to start hard example mining
            max_hard_ratio: Maximum fraction of hard examples per batch
            update_frequency: How often to update difficulty scores
            confidence_range: Range of confidences considered "uncertain"
            loss_percentile: Percentile for loss-based hard example identification
            momentum: Smoothing factor for difficulty score updates
        """
        ...
        # Track sample difficulties across epochs
        self.sample_difficulties = {}  # sample_id -> difficulty_score
        self.sample_losses = {}        # sample_id -> recent_loss
        self.sample_confidences = {}   # sample_id -> recent_confidence
        self.epoch_count = 0
        
        print(f"🔥 Hard Example Mining Initialized:")
        print(f"   Method: {mining_method}")
        print(f"   Difficulty threshold: {difficulty_threshold}")
        print(f"   Mining starts at epoch: {mining_start_epoch}")
        print(f"   Max hard ratio per batch: {max_hard_ratio}")
        print(f"   Confidence range for uncertain samples: {confidence_range}")
        
    def update_sample_statistics(self, sample_ids, losses, confidences):
        """
        Update difficulty statistics for samples after a training batch.
        
        Args:
            sample_ids: List of sample identifiers
            losses: Tensor of loss values for each sample
            confidences: Tensor of model confidence (probability) for each sample
        """
        if self.epoch_count < self.mining_start_epoch:
            return  # Don't track until mining starts
            
        losses_np = losses.detach().cpu().numpy()
        confidences_np = confidences.detach().cpu().numpy()
        
        for sample_id, loss, confidence in zip(sample_ids, losses_np, confidences_np):
            # Update loss history with momentum
            if sample_id in self.sample_losses:
                self.sample_losses[sample_id] = (
                    self.momentum * self.sample_losses[sample_id] + 
                    (1 - self.momentum) * loss
                )
            else:
                self.sample_losses[sample_id] = loss
                
            # Update confidence history with momentum  
            if sample_id in self.sample_confidences:
                self.sample_confidences[sample_id] = (
                    self.momentum * self.sample_confidences[sample_id] + 
                    (1 - self.momentum) * confidence
                )
            else:
                self.sample_confidences[sample_id] = confidence
                
    def compute_difficulty_scores(self):
        """
        Compute difficulty scores for all tracked samples based on the mining method.
        """
        if self.epoch_count < self.mining_start_epoch:
            return
        ...
        if self.mining_method == 'loss_based':
            self._compute_loss_based_difficulty()
        elif self.mining_method == 'confidence_based':
            self._compute_confidence_based_difficulty()
        elif self.mining_method == 'hybrid':
            self._compute_hybrid_difficulty()
            
        print(f"📊 Hard Example Mining Stats (Epoch {self.epoch_count}):")
        if self.sample_difficulties:
            difficulties = list(self.sample_difficulties.values())
            hard_count = sum(1 for d in difficulties if d > self.difficulty_threshold)
            print(f"   Total tracked samples: {len(difficulties):,}")
            print(f"   Hard examples: {hard_count:,} ({hard_count/len(difficulties)*100:.1f}%)")
            print(f"   Avg difficulty: {np.mean(difficulties):.3f}")
            print(f"   Difficulty range: {np.min(difficulties):.3f} - {np.max(difficulties):.3f}")
        
    def _compute_loss_based_difficulty(self):
        """Compute difficulty based on loss percentiles."""
        losses = list(self.sample_losses.values())
        if len(losses) == 0:
            return
            
        loss_threshold = np.percentile(losses, self.loss_percentile)
        
        for sample_id, loss in self.sample_losses.items():
            # Higher loss = higher difficulty
            difficulty = min(loss / loss_threshold, 2.0)  # Cap at 2x threshold
            self.sample_difficulties[sample_id] = difficulty
            
    def _compute_confidence_based_difficulty(self):
        """Compute difficulty based on prediction confidence (uncertainty)."""
        for sample_id, confidence in self.sample_confidences.items():
            # Samples with confidence in uncertain range are "hard"
            # Distance from 0.5 (maximum uncertainty) determines difficulty
            uncertainty = 1.0 - abs(confidence - 0.5) * 2  # Convert to [0,1] where 1 = most uncertain
            
            # Scale to difficulty score
            if self.confidence_range[0] <= confidence <= self.confidence_range[1]:
                # In uncertain range - high difficulty
                difficulty = 0.5 + uncertainty * 0.5  # Range [0.5, 1.0]
            else:
                # Outside uncertain range - lower difficulty
                difficulty = uncertainty * 0.5  # Range [0.0, 0.5]
                
            self.sample_difficulties[sample_id] = difficulty
            
    def _compute_hybrid_difficulty(self):
        """Compute difficulty using both loss and confidence."""
        # First compute individual components
        self._compute_loss_based_difficulty()
        
        # Store loss-based difficulties temporarily
        loss_difficulties = self.sample_difficulties.copy()
        
        # Compute confidence-based difficulties
        self._compute_confidence_based_difficulty()
        
        # Combine both with weighting
        for sample_id in self.sample_difficulties:
            if sample_id in loss_difficulties:
                loss_diff = loss_difficulties[sample_id]
                conf_diff = self.sample_difficulties[sample_id]
                
                # Weighted combination (70% confidence, 30% loss)
                self.sample_difficulties[sample_id] = 0.7 * conf_diff + 0.3 * loss_diff
                
    def get_hard_example_weights(self, sample_ids):
        """
        Get sampling weights for hard example mining.
        
        Args:
            sample_ids: List of sample identifiers for current batch
            
        Returns:
            numpy.array: Sampling weights (higher = more likely to be selected)
        """
        if self.epoch_count < self.mining_start_epoch or len(self.sample_difficulties) == 0:
            # Before mining starts, use uniform weights
            return np.ones(len(sample_ids))
            
        weights = []
        for sample_id in sample_ids:
            if sample_id in self.sample_difficulties:
                difficulty = self.sample_difficulties[sample_id]
                # Convert difficulty to sampling weight
                # Hard examples get higher weights
                weight = 1.0 + difficulty  # Range [1.0, 3.0] typically
            else:
                weight = 1.0  # Default weight for new samples
    ...
    def on_epoch_start(self, epoch):
        ...
        # Compute difficulty scores periodically
        if epoch >= self.mining_start_epoch and epoch % self.update_frequency == 0:
            self.compute_difficulty_scores()
            
    def is_mining_active(self):
        """Check if hard example mining is currently active."""
        return self.epoch_count >= self.mining_start_epoch


class HardExampleDataLoader:
    """
    🎯 DATA LOADER WITH HARD EXAMPLE MINING
    
    Wraps a standard DataLoader to implement hard example mining by adjusting
    sampling probabilities based on sample difficulty.
    """
    
    def __init__(self, dataset, miner, batch_size=32, shuffle=True, **kwargs):
        """
        Initialize hard example mining data loader.
        
        Args:
            dataset: Dataset to sample from
            miner: HardExampleMiner instance
            batch_size: Batch size
            shuffle: Whether to shuffle (will be overridden by mining)
            **kwargs: Additional arguments for DataLoader
        """
        self.dataset = dataset
        self.miner = miner
        self.batch_size = batch_size
        self.base_shuffle = shuffle
        self.dataloader_kwargs = kwargs
        
        # Create base dataloader
        self.base_dataloader = DataLoader(
            dataset, batch_size=batch_size, shuffle=False,  # We handle shuffling
            **kwargs
        )
        
        print(f"🎯 Hard Example DataLoader created:")
        print(f"   Dataset size: {len(dataset):,}")
        print(f"   Batch size: {batch_size}")
        print(f"   Base shuffle: {shuffle}")
    ...
    def __iter__(self):
        """
        Iterator that implements hard example mining by adjusting sampling probabilities.
        """
        # Get all sample indices
        all_indices = list(range(len(self.dataset)))
        
        if not self.miner.is_mining_active() or not self.base_shuffle:
            # Use standard sampling when mining is not active
            if self.base_shuffle:
                random.shuffle(all_indices)
                
            # Yield batches normally
            ...
            # Get sample IDs for all indices
            sample_ids = []
            for idx in all_indices:
                sample = self.dataset[idx]
                sample_ids.append(sample.get('file_id', f'sample_{idx}'))
                
            # Get mining weights
            weights = self.miner.get_hard_example_weights(sample_ids)
            
            # Create probability distribution
            probabilities = weights / np.sum(weights)
            
            # Sample with replacement using mining probabilities
            total_samples = len(all_indices)
            batches_needed = (total_samples + self.batch_size - 1) // self.batch_size
            
            for _ in range(batches_needed):
                # Sample batch_size indices according to mining probabilities
                batch_indices = np.random.choice(
                    all_indices, 
                    size=min(self.batch_size, total_samples),
                    p=probabilities,
                    replace=True  # Allow hard examples to appear multiple times
                )
    ...
    def _collate_batch(self, batch):
        """Collate a batch of samples using the custom collate function."""
        return custom_collate_fn(batch)


class TIMITCustomSplitter:
    """
    🎲 CUSTOM TIMIT DATASET SPLITTER
    
    Creates custom train/validation/test splits from the complete TIMIT dataset
    with speaker independence and dialect balance preservation.
    """
    
    def __init__(self, 
                 split_ratios=(0.7, 0.15, 0.15),  # train, val, test
                 ensure_speaker_independence=True,
                 balance_dialects=True,
                 random_seed=42):
        """
        Initialize the custom TIMIT splitter.
        
        Args:
            split_ratios: Tuple of (train_ratio, val_ratio, test_ratio)
            ensure_speaker_independence: Ensure no speaker overlap between splits
            balance_dialects: Maintain dialect region balance across splits
            random_seed: Random seed for reproducible splits
        """
        self.split_ratios = split_ratios
        self.ensure_speaker_independence = ensure_speaker_independence
        self.balance_dialects = balance_dialects
        ...
        print(f"🎲 Custom TIMIT Splitter initialized:")
        print(f"   Split ratios: Train={split_ratios[0]:.1%}, Val={split_ratios[1]:.1%}, Test={split_ratios[2]:.1%}")
        print(f"   Speaker independence: {ensure_speaker_independence}")
        print(f"   Dialect balance: {balance_dialects}")
        print(f"   Random seed: {random_seed}")
        
    def create_custom_splits(self, complete_timit_data):
        """
        Create custom train/validation/test splits from complete TIMIT data.
        
        Args:
            complete_timit_data: Complete TIMIT dataset (all samples)
            
        Returns:
            tuple: (train_data, val_data, test_data)
        """
        print(f"\n🎲 Creating custom TIMIT splits...")
        print(f"   Input data size: {len(complete_timit_data):,} samples")
        
        # Set random seed for reproducibility
        np.random.seed(self.random_seed)
        random.seed(self.random_seed)
        
        if self.ensure_speaker_independence:
            return self._create_speaker_independent_splits(complete_timit_data)
        else:
            return self._create_random_splits(complete_timit_data)
            
    def _create_speaker_independent_splits(self, data):
        """Create splits ensuring no speaker overlap between train/val/test."""
        print("   📊 Creating speaker-independent splits...")
        
        # Extract speaker information from file IDs
        speaker_info = {}
        ...
        for idx, item in enumerate(data):
            ...
            # Format: DR#/SPEAKER/filename or just SPEAKER
            if '/' in file_id:
                parts = file_id.split('/')
                speaker_id = parts[-2] if len(parts) >= 2 else parts[-1]
                dialect_region = parts[-3] if len(parts) >= 3 and parts[-3].startswith('DR') else 'DR0'
            else:
                # Fallback: use first 5 characters as speaker ID
                speaker_id = file_id[:5] if len(file_id) >= 5 else file_id
                dialect_region = 'DR0'
            ...
            speaker_info[speaker_id]['indices'].append(idx)
            speaker_info[speaker_id]['num_samples'] += 1
            
        print(f"   📊 Found {len(speaker_info)} unique speakers")
        
        # Group speakers by dialect if balancing is enabled
        if self.balance_dialects:
            dialect_speakers = {}
            for speaker_id, info in speaker_info.items():
                dialect = info['dialect']
                if dialect not in dialect_speakers:
                    dialect_speakers[dialect] = []
                dialect_speakers[dialect].append(speaker_id)
                
            print(f"   📊 Found {len(dialect_speakers)} dialect regions")
            for dialect, speakers in dialect_speakers.items():
                print(f"      {dialect}: {len(speakers)} speakers")
                
            # Split speakers within each dialect
            train_speakers, val_speakers, test_speakers = [], [], []
            
            for dialect, speakers in dialect_speakers.items():
                random.shuffle(speakers)
                
                n_train = int(len(speakers) * self.split_ratios[0])
                n_val = int(len(speakers) * self.split_ratios[1])
            ...
            # Simple random split of speakers
            all_speakers = list(speaker_info.keys())
            random.shuffle(all_speakers)
            
            n_train = int(len(all_speakers) * self.split_ratios[0])
            n_val = int(len(all_speakers) * self.split_ratios[1])
        ...
        for speaker in train_speakers:
            train_indices.extend(speaker_info[speaker]['indices'])
        for speaker in val_speakers:
            val_indices.extend(speaker_info[speaker]['indices'])
        for speaker in test_speakers:
            test_indices.extend(speaker_info[speaker]['indices'])
        ...
        print(f"   ✅ Speaker-independent splits created:")
        ...
        print(f"   ✅ Speaker independence verified!")
        
        return train_data, val_data, test_data
        
    def _create_random_splits(self, data):
        """Create random splits without speaker independence constraints."""
        print("   🎲 Creating random splits...")
        
        indices = list(range(len(data)))
        random.shuffle(indices)
        
        n_train = int(len(indices) * self.split_ratios[0])
        n_val = int(len(indices) * self.split_ratios[1])
...
class SafeBCEWithLogitsLoss(nn.Module):
    ...
    FIXED: Corrected weight calculation that was causing recall=1.0 issue.
    """
    ...
    def forward(self, logits, targets):
        ...
        # FIXED: Use proper pos_weight parameter instead of manual weighting
        # This was the bug causing recall=1.0 - we were weighting based on targets > threshold
        pos_weight_tensor = torch.tensor(self.pos_weight, device=logits.device, dtype=logits.dtype)
        
        # Use standard BCE with proper pos_weight
...
def train_local_classifier(model, train_dataloader, val_dataloader, device, config, num_epochs=10, 
                          ...
                          label_smoothing=0.0, hard_example_miner=None):
    """
    ...
    if hard_example_miner is not None:
        ...
        print(f"   Mining method: {hard_example_miner.mining_method}")
        print(f"   Mining starts at epoch: {hard_example_miner.mining_start_epoch}")
    else:
        print("📊 Standard training (no hard example mining)")
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        ...
        factor=0.75,         # Gentler reduction (25% vs 50%)
        patience=4,          # More patience
        threshold=0.005,     # Only reduce if improvement < 0.5%
    ...
    for epoch in range(num_epochs):
        ...
        # Update hard example miner for new epoch
        if hard_example_miner is not None:
            hard_example_miner.on_epoch_start(epoch)
            if hard_example_miner.is_mining_active():
                print(f"🔥 Hard example mining ACTIVE (epoch {epoch+1})")
            else:
                print(f"📊 Hard example mining warming up... (starts at epoch {hard_example_miner.mining_start_epoch})")
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            file_ids = batch['file_id']  # For hard example mining
            ...
            if use_mixed_precision and scaler is not None:
                ...
                # Calculate loss for the full batch
                loss = criterion(logits, labels)  # Safe BCEWithLogitsLoss
                
                # Update hard example mining statistics if enabled
                if hard_example_miner is not None and hard_example_miner.is_mining_active():
                    with torch.no_grad():
                        # Get per-sample losses for mining
                        individual_losses = F.binary_cross_entropy_with_logits(
                            logits, labels, reduction='none'
                        )
                        # Get confidences (probabilities)
                        confidences = torch.sigmoid(logits)
                        
                        # Update mining statistics
                        hard_example_miner.update_sample_statistics(file_ids, individual_losses, confidences)
...
def load_timit_data_for_local_windows(split='train', max_samples=None, 
                                      use_custom_split=False, custom_splitter=None):
    """
    Load TIMIT data for local window approach with optional custom splitting.
    
    Args:
        split: 'train', 'test', 'val', or 'complete' for custom splitting
        max_samples: Maximum number of samples to load (None = all)
        use_custom_split: Whether to use custom splitting instead of official TIMIT splits
        custom_splitter: TIMITCustomSplitter instance (required if use_custom_split=True)
        
    Returns:
        Loaded TIMIT data for the specified split
    """
    print(f"\n🚨 LOADING TIMIT DATA DEBUG:")
    print(f"   Split: {split}")
    print(f"   Max samples: {max_samples}")
    print(f"   Use custom split: {use_custom_split}")
    ...
    try:
        ...
        if use_custom_split:
            if custom_splitter is None:
                raise ValueError("custom_splitter is required when use_custom_split=True")
                
            print(f"   🎲 Using custom TIMIT splitting...")
            
            # Load complete TIMIT dataset (both train and test)
            print(f"   📦 Loading complete TIMIT dataset...")
            train_data = load_data('train', None)  # Load all train data
            test_data = load_data('test', None)    # Load all test data
            
            # Combine train and test for custom splitting
            complete_data = train_data + test_data
            print(f"   📊 Complete dataset size: {len(complete_data)} samples")
            print(f"   📊 Original train: {len(train_data)}, Original test: {len(test_data)}")
            
            # Create custom splits
            custom_train, custom_val, custom_test = custom_splitter.create_custom_splits(complete_data)
            ...
            if split == 'train':
                ...
                raise ValueError(f"Invalid split '{split}' for custom splitting. Use 'train', 'val', 'test', or 'complete'")
                
            print(f"   🎲 Custom {split} split loaded: {len(data)} samples")
            
        else:
            # Use official TIMIT splits
            print(f"   📂 Using official TIMIT splits...")
...
def main():
    """
    Main training pipeline with optimized settings and hard example mining.
    """
    config = {
        # CRITICAL PARAMETERS FOR POSITIVE PREDICTIONS - FIXED TO STABLE SETTINGS
        'pos_weight': 2.0,                   # STABLE: Conservative positive bias
        'threshold': 0.5,                    # STABLE: Standard probability threshold
        'label_smoothing': 0.0,              # STABLE: No label smoothing
        
        # HARD EXAMPLE MINING SETTINGS - DISABLED FOR STABILITY
        'use_hard_example_mining': True,    # DISABLED: Use simple training for now
        'mining_method': 'hybrid',           # 'confidence_based', 'loss_based', 'hybrid'
        'mining_start_epoch': 5,             # Start later when model is more stable
        'difficulty_threshold': 0.5,         # Lower threshold = more samples considered hard
        'max_hard_ratio': 0.6,               # Allow more hard examples per batch
        'confidence_range': (0.2, 0.8),      # Wider uncertainty range
        
        # CUSTOM SPLIT SETTINGS
        'use_custom_split': False,           # Enable custom TIMIT splitting
        'split_ratios': (0.7, 0.15, 0.15),  # Train, Val, Test ratios
        'ensure_speaker_independence': True, # No speaker overlap between splits
        'balance_dialects': True,            # Balance dialect regions
        'custom_split_seed': 42,             # Seed for reproducible splits
        
        # STABLE TRAINING PARAMETERS
        'batch_size': 32,                    # Standard batch size
        'num_epochs': 20,                    # Conservative epoch count
        'learning_rate': 1e-4,               # STABLE: Conservative learning rate
        'weight_decay': 1e-4,                # STABLE: Standard regularization
        
        # NO POOLING MODEL ARCHITECTURE 
        'hidden_dim': 512,                   # NO POOLING: Adequate capacity for temporal patterns
        'dropout_rate': 0.3,                 # NO POOLING: Standard dropout
    ...
    # Initialize custom splitter if requested
    custom_splitter = None
    if config['use_custom_split']:
        print("\n🎲 Initializing custom TIMIT splitter...")
        custom_splitter = TIMITCustomSplitter(
            split_ratios=config['split_ratios'],
            ensure_speaker_independence=config['ensure_speaker_independence'],
            balance_dialects=config['balance_dialects'],
            random_seed=config['custom_split_seed']
        )
    ...
    if config['use_custom_split']:
        ...
        train_data = load_timit_data_for_local_windows(
            'train', None, use_custom_split=True, custom_splitter=custom_splitter
        )
        val_data = load_timit_data_for_local_windows(
            'val', None, use_custom_split=True, custom_splitter=custom_splitter
        )
        test_data = load_timit_data_for_local_windows(
            'test', None, use_custom_split=True, custom_splitter=custom_splitter
        )
        print(f"📊 Custom split sizes: Train={len(train_data)}, Val={len(val_data)}, Test={len(test_data)}")
    else:
        # Use official TIMIT splits
        ...
        val_data = None  # Will create from train_data later
        print(f"📊 Official split sizes: Train={len(train_data)}, Test={len(test_data)}")
    ...
    # Preprocess validation data if using custom splits
    val_metadata = None
    val_preprocessing_time = 0
    if config['use_custom_split'] and val_data is not None:
        print(f"\n📦 Preprocessing VALIDATION data...")
        val_preprocessor = WindowPreprocessor(
            val_data, processor,
            window_duration=config['window_duration'],
            boundary_tolerance=config['boundary_tolerance'],
            negative_exclusion_zone=config['negative_exclusion_zone'],
            negative_sampling_ratio=config['negative_sampling_ratio'],
            save_dir=config['train_preprocessed_dir'] + '_val',  # Separate directory
            max_windows_per_file=config['max_windows_per_file'],
            max_positive_per_file=config['max_positive_per_file'],
            max_negative_per_file=config['max_negative_per_file']
        )
        val_start = time.time()
        val_metadata = val_preprocessor.preprocess_all_windows(force_reprocess=config['force_reprocess'])
        val_preprocessing_time = time.time() - val_start
    ...
    total_preprocessing_time = train_preprocessing_time + test_preprocessing_time + val_preprocessing_time
    print(f"\n✅ PREPROCESSING COMPLETE!")
    print(f"   Train preprocessing: {train_preprocessing_time/60:.1f} minutes")
    if val_preprocessing_time > 0:
        print(f"   Validation preprocessing: {val_preprocessing_time/60:.1f} minutes")
    ...
    # Initialize hard example miner if requested
    hard_example_miner = None
    if config['use_hard_example_mining']:
        print("\n🔥 Initializing hard example miner...")
        hard_example_miner = HardExampleMiner(
            mining_method=config['mining_method'],
            difficulty_threshold=config['difficulty_threshold'],
            mining_start_epoch=config['mining_start_epoch'],
            max_hard_ratio=config['max_hard_ratio'],
    ...
    if config['use_custom_split'] and val_data is not None and val_metadata is not None:
        # Use dedicated validation split
        val_dataset = PreprocessedWindowDataset(
            val_metadata,
            save_dir=config['train_preprocessed_dir'] + '_val'  # Use validation directory
        )
        train_subset = train_dataset
        val_subset = val_dataset
        print(f"📊 Using dedicated validation split: {len(val_subset)} samples")
    else:
        # Split train for validation (traditional approach)
        val_size = len(train_dataset) // 5
        train_size = len(train_dataset) - val_size
        train_subset, val_subset = torch.utils.data.random_split(train_dataset, [train_size, val_size])
        print(f"📊 Created validation split from training: {len(val_subset)} samples")
    
    # Create data loaders with optional hard example mining
    if config['use_hard_example_mining'] and hard_example_miner is not None:
        print("🔥 Creating hard example mining data loader...")
        train_dataloader = HardExampleDataLoader(
            train_subset, hard_example_miner,
            batch_size=config['batch_size'], shuffle=True,
            collate_fn=custom_collate_fn, num_workers=8, pin_memory=True
        )
    else:
        print("📊 Creating standard data loader...")
    ...
    try:
        # Use NO POOLING architecture - POOLING IS RETARDED FOR BOUNDARY DETECTION!
        model = NoPoolingBoundaryClassifier(
            freeze_wav2vec2=True,
            hidden_dim=config['hidden_dim'],        # Hidden dimensions
            dropout_rate=config['dropout_rate']     # Dropout rate
        )
        print("   ✅ NO POOLING model architecture created successfully")
        ...
        print("   🔄 Trying with minimal NO POOLING architecture...")
        try:
            # Try with minimal no pooling model if base fails
            model = NoPoolingBoundaryClassifier(
                wav2vec2_model_name="facebook/wav2vec2-base", 
                freeze_wav2vec2=True,
                hidden_dim=128,  # Minimal but still no pooling
                dropout_rate=0.3
            )
            print("   ✅ Model created with minimal NO POOLING architecture")
    ...
    history = train_local_classifier(
        ...
        hard_example_miner=hard_example_miner  # Pass hard example miner
    )
    ...
    print("=" * 80)
    print(f"✨ NO POOLING LOCAL WINDOW APPROACH WITH FIXED ARCHITECTURE:")
    print(f"   🎯 Clear binary classification task (not sparse sequence)")
    print(f"   🔥 Balanced training data (50/50 vs previous 79/21)")
    print(f"   🔥 NO POOLING ARCHITECTURE - Pure temporal modeling preserves ALL information")
    print(f"   ⚡ Direct boundary predictions with temporal LSTM + boundary region focus")
    print(f"   🎯 FIXED 0.5 threshold for proper probability calibration")
    print(f"   🔧 FIXED SafeBCEWithLogitsLoss - proper pos_weight usage")
    print(f"   🚫 ZERO pooling operations - pooling destroys temporal precision!")
    if config['use_hard_example_mining']:
        print(f"   🔥 HARD EXAMPLE MINING: {config['mining_method']} method")
        print(f"   📊 Mining started at epoch {config['mining_start_epoch']}")
    if config['use_custom_split']:
        print(f"   🎲 CUSTOM TIMIT SPLITS: Speaker-independent with dialect balance")
