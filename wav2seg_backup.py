...
class BoundaryDetectionLoss(nn.Module):
    """
    Revolutionary boundary detection loss function that operates on actual boundary positions
    instead of frame-level classification. This treats boundary detection as a temporal
    localization problem rather than a binary classification problem.
    ...
    3. Heavily penalizes missing boundaries
    4. Uses tolerance-based matching for realistic evaluation
    """
    
    def __init__(self, 
                 distance_weight=1.0,           # Weight for temporal distance penalty
                 missing_penalty=15.0,          # Heavy penalty for missing boundaries
                 false_positive_penalty=3.0,    # Penalty for false positives
                 tolerance_frames=320,          # 20ms tolerance at 16kHz (20ms * 16000 / 1000)
                 min_distance_frames=160):      # Minimum distance between boundaries (10ms)
        """
        Initialize the boundary detection loss.
        
        Args:
            distance_weight: Weight for temporal distance penalty
            missing_penalty: Heavy penalty for missing boundaries  
            false_positive_penalty: Penalty for false positives
            tolerance_frames: Tolerance window for boundary matching
            min_distance_frames: Minimum distance between predicted boundaries
        """
        super().__init__()
    ...
    def extract_boundaries_from_logits(self, logits, threshold=0.0):
        """
        Extract boundary positions from model logits using peak detection.
        ...
        Returns:
            List of boundary frame positions
        """
        ...
        for i in range(1, len(probs_np) - 1):
            if (probs_np[i] > threshold and 
                probs_np[i] > probs_np[i-1] and 
                probs_np[i] > probs_np[i+1]):
                boundaries.append(i)
        ...
        if len(boundaries) > 1:
            ...
            for boundary in boundaries[1:]:
                if boundary - filtered_boundaries[-1] >= self.min_distance_frames:
                    filtered_boundaries.append(boundary)
            boundaries = filtered_boundaries
            
        return boundaries
    
    def extract_boundaries_from_labels(self, labels):
        """
        Extract boundary positions from binary labels.
        
        Args:
            labels: Binary boundary labels [seq_len]
        ...
        # Find transitions from 0 to 1 (boundary starts)
        labels_np = labels.detach().cpu().numpy()
        
        # Look for center of boundary regions
        in_boundary = False
        boundary_start = 0
        
        for i in range(len(labels_np)):
            if labels_np[i] > 0.5 and not in_boundary:
                # Start of boundary region
                boundary_start = i
                in_boundary = True
            elif labels_np[i] <= 0.5 and in_boundary:
                # End of boundary region - take center
                boundary_center = (boundary_start + i - 1) // 2
                boundaries.append(boundary_center)
                in_boundary = False
        
        # Handle case where boundary region extends to end
        if in_boundary:
            boundary_center = (boundary_start + len(labels_np) - 1) // 2
            boundaries.append(boundary_center)
            
        return boundaries
    
    def calculate_boundary_matching_loss(self, pred_boundaries, true_boundaries):
        """
        Calculate loss based on boundary position matching.
        
        Args:
            pred_boundaries: List of predicted boundary positions
        ...
        if len(true_boundaries) == 0:
            # No true boundaries - penalize any predictions
            return self.false_positive_penalty * len(pred_boundaries)
        
        if len(pred_boundaries) == 0:
            # No predictions - heavily penalize missing boundaries
            return self.missing_penalty * len(true_boundaries)
        
        total_loss = 0.0
        used_predictions = set()
        
        # For each true boundary, find best matching prediction
        for true_boundary in true_boundaries:
            ...
            for i, pred_boundary in enumerate(pred_boundaries):
                if i not in used_predictions:
                    ...
                    if distance <= self.tolerance_frames and distance < best_distance:
                        best_match = i
            ...
            if best_match is not None:
                ...
                # Small penalty based on temporal distance
                total_loss += self.distance_weight * (best_distance / self.tolerance_frames)
            else:
                # No match found - missing boundary penalty
                total_loss += self.missing_penalty
        
        # Penalize unmatched predictions (false positives)
        false_positives = len(pred_boundaries) - len(used_predictions)
        total_loss += self.false_positive_penalty * false_positives
        
        return total_loss
    
    def forward(self, logits, labels):
        """
        Calculate boundary detection loss using a differentiable approximation.
        ...
        Returns:
            torch.Tensor: Boundary detection loss
        """
        # For the loss computation, we need to maintain differentiability
        # We'll use a differentiable approximation based on the original concepts
        ...
        # For each sample, estimate the boundary detection quality
        ...
        for i in range(batch_size):
            ...
            # Differentiable boundary counting approximation
            # Use soft peak detection based on probability gradients
            
            # Calculate first-order differences to find peaks
            if len(sample_probs) > 2:
                diff = sample_probs[1:-1] - sample_probs[:-2]  # Forward difference
                diff2 = sample_probs[2:] - sample_probs[1:-1]  # Backward difference
                
                # Soft peak detection: high probability with positive then negative gradient
                peak_scores = torch.relu(sample_probs[1:-1]) * torch.relu(diff) * torch.relu(-diff2)
                
                # Boundary count approximation (soft count)
                pred_boundary_count = torch.sum(peak_scores)
            else:
                pred_boundary_count = torch.sum(sample_probs)
            
            # True boundary count (hard count, but differentiable through labels)
            true_boundary_count = torch.sum(sample_labels)
            
            # Boundary count penalty (differentiable)
            count_diff = torch.abs(pred_boundary_count - true_boundary_count)
            
            # Missing boundary penalty: heavily penalize when predicted count << true count
            if true_boundary_count > 0:
                count_ratio = pred_boundary_count / true_boundary_count
                missing_penalty = self.missing_penalty * torch.relu(1.0 - count_ratio) ** 2
            else:
                missing_penalty = torch.tensor(0.0, device=logits.device)
            
            # False positive penalty: penalize when predicted count >> true count
            if true_boundary_count > 0:
                excess_ratio = torch.relu(pred_boundary_count - true_boundary_count) / true_boundary_count
                fp_penalty = self.false_positive_penalty * excess_ratio
            else:
                fp_penalty = self.false_positive_penalty * pred_boundary_count
            
            # Distance penalty: encourage predicted boundaries to be close to true boundaries
            # Use a differentiable approximation of boundary alignment
            distance_penalty = torch.tensor(0.0, device=logits.device)
            if true_boundary_count > 0:
                # Create a soft alignment penalty
                for t in range(len(sample_labels)):
                    if sample_labels[t] > 0.5:  # True boundary position
                        # Calculate soft distance to nearest predicted boundary
                        # Use exponential weighting to focus on nearby predictions
                        weights = torch.exp(-torch.abs(torch.arange(len(sample_probs), device=logits.device, dtype=torch.float) - t) / self.tolerance_frames)
                        alignment_score = torch.sum(weights * sample_probs)
                        distance_penalty += self.distance_weight * torch.relu(1.0 - alignment_score)
            
            # Combine penalties
            sample_loss = missing_penalty + fp_penalty + distance_penalty
...
def train(model, train_dataloader, val_dataloader, device, num_epochs=10, learning_rate=1e-4):
    ...
    criterion = BoundaryDetectionLoss(
        ...
        missing_penalty=15.0,          # Heavy penalty for missing boundaries
        false_positive_penalty=3.0,    # Penalty for false positives
        tolerance_frames=320,          # 20ms tolerance at 16kHz
        min_distance_frames=160        # Minimum distance between boundaries (10ms)
    )
    
    print(f"📊 Revolutionary Loss Configuration:")
    print(f"   Missing boundary penalty: {15.0}x")
    print(f"   False positive penalty: {3.0}x")
    print(f"   Distance weight: {1.0}x")
    print(f"   Tolerance: 320 frames (20ms)")
    print(f"   Minimum boundary distance: 160 frames (10ms)")
    print(f"   🎯 This directly optimizes boundary positions and temporal accuracy!")
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                for i in range(logits.size(0)):
                    # Extract boundaries using the loss function's methods
                    pred_boundaries = criterion.extract_boundaries_from_logits(logits[i])
                    true_boundaries = criterion.extract_boundaries_from_labels(labels[i])
                    ...
                    for true_boundary in true_boundaries:
                        for j, pred_boundary in enumerate(pred_boundaries):
                            if j not in used_predictions:
                                distance = abs(pred_boundary - true_boundary)
                                if distance <= criterion.tolerance_frames:
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            for i in range(logits.size(0)):
                ...
                # Extract boundaries using the revolutionary approach
                if criterion and hasattr(criterion, 'extract_boundaries_from_logits'):
                    # Use the same methods as the loss function for consistency
                    pred_boundaries = criterion.extract_boundaries_from_logits(sample_logits)
                    true_boundaries = criterion.extract_boundaries_from_labels(sample_labels)
                else:
                    # Fallback to traditional approach
                    pred = torch.sigmoid(sample_logits) > 0.5
                    pred_boundaries = find_boundaries_from_predictions(pred.cpu().numpy())
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_dataloader):
            ...
            for i in range(len(file_ids)):
                ...
                # Find predicted boundaries (peaks in prediction)
                pred_boundaries = find_boundaries_from_predictions(pred)
...
def find_boundaries_from_predictions(predictions, min_distance=160):
    """
    Extract boundary positions from binary predictions.
...
def plot_results(results, save_path='evaluation_plots.png'):
    ...
    axes[0, 0].hist(maes, bins=30, alpha=0.7, color='skyblue', edgecolor='black')
    axes[0, 0].set_title('Distribution of Mean Absolute Error (MAE)')
    axes[0, 0].set_xlabel('MAE (frames)')
    axes[0, 0].set_ylabel('Frequency')
    axes[0, 0].axvline(np.mean(maes), color='red', linestyle='--', label=f'Mean: {np.mean(maes):.2f}')
    axes[0, 0].legend()
