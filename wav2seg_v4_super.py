...
class CompetitiveWindowPreprocessor:
    ...
    def _save_negative_windows(self, audio, boundary_positions, file_id, sample_idx, metadata):
        ...
        Also stores distance to nearest boundary for hard example mining.
        """
        ...
        for start_pos in range(0, len(audio) - self.window_samples + 1):
            end_pos = start_pos + self.window_samples
            # Use the NEGATIVE exclusion zone (40ms) - much larger than positive zone
            negative_critical_start = end_pos - self.negative_exclusion_samples
            
            # Check the NEGATIVE critical region (last 40ms)
            ...
            for boundary_pos in boundary_positions:
                if negative_critical_start <= boundary_pos < end_pos:
                    ...
                    break
                # Calculate minimum distance to this boundary (in seconds)
                distance = abs(boundary_pos - end_pos) / self.sample_rate
                min_distance = min(min_distance, distance)
            
            # If no boundary in the 40ms exclusion zone, it's a valid negative window
...
def main():
    ...
    config = {
        ...
        'force_reprocess': False,             # FORCE reprocess for competitive features
