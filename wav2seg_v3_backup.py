...
✅ PROVEN VOCAL FEATURES: Energy, MFCCs, spectral centroid/rolloff, ZCR (17 features)
...
- Prosodic feature integration (energy, spectral, ZCR deltas)
...
class CompetitiveWindowPreprocessor:
    ...
    def __init__(self, data, processor, window_duration=0.5, sample_rate=16000, 
                 ...
                 use_prosodic=True, use_gaussian_targets=False):
        """
        ...
        - PROSODIC FEATURES: Energy deltas, spectral centroids, ZCR changes
    ...
    def extract_prosodic_features(self, audio):
        """
        Extract PROVEN vocal features that actually help boundary detection.
        
        Based on phonetic research, these features are most relevant:
        - Energy changes (volume transitions)
        - MFCC features (spectral shape)
        ...
        try:
            ...
            # 1. Energy features (most important for boundaries)
            energy = librosa.feature.rms(y=audio, hop_length=hop_length)[0]
            energy_delta = np.diff(energy, prepend=energy[0])
            energy_delta2 = np.diff(energy_delta, prepend=energy_delta[0])
            
            # 2. MFCC features (spectral shape - proven for speech)
            mfccs = librosa.feature.mfcc(y=audio, sr=self.sample_rate, n_mfcc=n_mfcc, 
                                       hop_length=hop_length)
            # Use first 6 MFCCs (most informative for phonetic boundaries)
            mfcc_mean = np.mean(mfccs[:6], axis=1)
            mfcc_std = np.std(mfccs[:6], axis=1)
            ...
            # Calculate deltas for temporal dynamics
            ...
            # Compile features (total: 17 features)
            prosodic_features = {
                # Energy features (3)
                'energy_delta': resample_feature(energy_delta, target_length),
                'energy_delta2': resample_feature(energy_delta2, target_length),
                'energy_mean': np.mean(energy),  # Global feature
                
                # MFCC features (12)
                'mfcc_mean': mfcc_mean,  # 6 features
                'mfcc_std': mfcc_std,    # 6 features
    ...
    def _save_positive_windows(self, audio, boundary_positions, file_id, sample_idx, metadata):
        """
        Generate COMPETITIVE positive windows - boundary in last ±20ms (competition standard).
        
        Strategy: For each boundary, create a window where the boundary falls in the 
        last ±20ms (320 samples) of the window to match competition evaluation.
        """
        saved_count = 0
        
        for boundary_pos in boundary_positions:
            # Position boundary in the last ±20ms region (COMPETITIVE)
            # Place boundary at window_end - (boundary_tolerance_samples // 2) for center of critical region
            boundary_offset_from_end = self.boundary_tolerance_samples // 2
            ...
            # Verify boundary is in the COMPETITIVE positive region (last ±20ms)
            positive_region_start = self.window_samples - self.boundary_tolerance_samples
            if not (positive_region_start <= boundary_pos_in_window < self.window_samples):
                continue  # Skip if boundary not in expected competitive region
            ...
            try:
                ...
                # Extract COMPETITIVE prosodic features
                prosodic_features = self.extract_prosodic_features(window_audio)
                ...
                window_data = {
                    ...
                    'file_id': file_id,
                    'prosodic_features': prosodic_features,  # COMPETITIVE FEATURE
                    'metadata': {
                        'boundary_pos': boundary_pos,
                        'boundary_pos_in_window': boundary_pos_in_window,
                        'window_start': actual_start,
                        'competitive_region_start': positive_region_start
                    }
                }
...
class CompetitiveMultiScaleBoundaryClassifier(nn.Module):
    """
    🏆 COMPETITIVE Multi-Scale Boundary Classifier inspired by 85-88% F1 competitors.
    
    COMPETITIVE FEATURES:
    - Multi-scale temporal convolutions (fine 3x3 + coarse 9x9)
    - Prosodic feature integration (energy, spectral, ZCR deltas)
    - Multi-head attention for feature fusion
    - Auxiliary boundary localization head
    """
    
    def __init__(self, wav2vec2_model_name="facebook/wav2vec2-base", freeze_wav2vec2=True, 
                 hidden_dim=256, dropout_rate=0.2, use_prosodic=True, num_attention_heads=8):
        """
        Initialize COMPETITIVE multi-scale boundary classifier.
        
        Args:
            wav2vec2_model_name: Pretrained Wav2Vec2 model
            freeze_wav2vec2: Whether to freeze Wav2Vec2 parameters
            hidden_dim: Hidden dimension (256 for competitive performance)
            dropout_rate: Dropout rate (0.2 for more capacity)
        ...
        print(f"      🏆 Building COMPETITIVE multi-scale architecture...")
        
        # COMPETITIVE MULTI-SCALE TEMPORAL CONVOLUTIONS
        # Fine-scale patterns (3x3 kernels) - captures rapid transitions
        self.fine_conv1 = nn.Conv1d(wav2vec2_hidden_size, hidden_dim, kernel_size=3, padding=1)
        self.fine_conv2 = nn.Conv1d(hidden_dim, hidden_dim//2, kernel_size=3, padding=1)
        
        # Coarse-scale patterns (9x9 kernels) - captures longer dependencies  
        self.coarse_conv1 = nn.Conv1d(wav2vec2_hidden_size, hidden_dim, kernel_size=9, padding=4)
        self.coarse_conv2 = nn.Conv1d(hidden_dim, hidden_dim//2, kernel_size=9, padding=4)
        
        # Intelligent prosodic feature integration (if enabled)
        prosodic_dim = 18 if use_prosodic else 0  # 17 features + energy_mean (global)
        if use_prosodic:
            # Separate processing for temporal and global features
            self.prosodic_temporal_projection = nn.Linear(15, hidden_dim//4)  # 15 temporal features
            self.prosodic_global_projection = nn.Linear(13, hidden_dim//8)   # 12 MFCC + 1 energy_mean
        
        # Multi-head attention for feature fusion
        prosodic_features_dim = (hidden_dim//4 + hidden_dim//8) if use_prosodic else 0
        total_features = hidden_dim + prosodic_features_dim  # Fine + coarse + prosodic
        ...
        print(f"      ✅ COMPETITIVE multi-scale boundary detection architecture built successfully")
        print(f"      🔧 Features: Multi-scale conv, Prosodic features, Multi-head attention")
        print(f"      🧠 Multi-head attention: Model learns which parts of window matter")
    ...
    def forward(self, input_values, prosodic_features=None):
        """
        🏆 COMPETITIVE forward pass with multi-scale temporal analysis.
        
        Args:
            input_values: Audio input [batch_size, seq_len]
            prosodic_features: Optional prosodic features dict
        ...
        with torch.no_grad():
            ...
            hidden_states = wav2vec2_outputs.last_hidden_state.detach()  # [batch, seq_len, hidden_dim]
        
        # Transpose for convolutions: [batch, hidden_dim, seq_len]
        x = hidden_states.transpose(1, 2)  # [batch, 768, seq_len]
        
        # COMPETITIVE MULTI-SCALE FEATURE EXTRACTION
        # Fine-scale patterns (rapid transitions)
        fine_features = F.relu(self.fine_conv1(x))  # [batch, hidden_dim, seq_len]
        fine_features = F.relu(self.fine_conv2(fine_features))  # [batch, hidden_dim//2, seq_len]
        
        # Coarse-scale patterns (longer dependencies)
        coarse_features = F.relu(self.coarse_conv1(x))  # [batch, hidden_dim, seq_len]
        coarse_features = F.relu(self.coarse_conv2(coarse_features))  # [batch, hidden_dim//2, seq_len]
        
        # Combine multi-scale features
        combined_features = torch.cat([fine_features, coarse_features], dim=1)  # [batch, hidden_dim, seq_len]
        combined_features = combined_features.transpose(1, 2)  # [batch, seq_len, hidden_dim]
        
        # Add intelligent prosodic features if available
        if self.use_prosodic and prosodic_features is not None:
            try:
                # Process temporal features (sequence-based)
                temporal_features = []
                for key in ['energy_delta', 'energy_delta2', 'centroid_delta', 'rolloff_delta', 'zcr_delta']:
                    if key in prosodic_features:
                        temporal_features.append(torch.tensor(prosodic_features[key], dtype=torch.float32))
                
                if temporal_features:
                    # Stack temporal features: [batch, seq_len, 5]
                    prosodic_temporal = torch.stack(temporal_features, dim=-1).to(combined_features.device)
                    
                    # Resize to match combined_features sequence length
                    seq_len = combined_features.shape[1]
                    if prosodic_temporal.shape[1] != seq_len:
                        prosodic_temporal = F.interpolate(
                            prosodic_temporal.transpose(1, 2), 
                            size=seq_len, 
                            mode='linear', 
                            align_corners=False
                        ).transpose(1, 2)
                    ...
                    if prosodic_temporal.shape[0] != batch_size:
                        prosodic_temporal = prosodic_temporal.expand(batch_size, -1, -1)
                    
                    # Project temporal features
                    prosodic_temporal_proj = self.prosodic_temporal_projection(prosodic_temporal)
                    combined_features = torch.cat([combined_features, prosodic_temporal_proj], dim=-1)
                
                # Process global features (window-level)
                global_features = []
                if 'energy_mean' in prosodic_features:
                    global_features.append(prosodic_features['energy_mean'])
                if 'mfcc_mean' in prosodic_features:
                    global_features.extend(prosodic_features['mfcc_mean'])
                if 'mfcc_std' in prosodic_features:
                    global_features.extend(prosodic_features['mfcc_std'])
                
                if global_features:
                    # Create global feature tensor: [batch, 13]
                    prosodic_global = torch.tensor(global_features, dtype=torch.float32).to(combined_features.device)
                    if prosodic_global.shape[0] != batch_size:
                        prosodic_global = prosodic_global.unsqueeze(0).expand(batch_size, -1)
                    
                    # Project and broadcast to sequence length
                    prosodic_global_proj = self.prosodic_global_projection(prosodic_global)  # [batch, hidden_dim//8]
                    prosodic_global_expanded = prosodic_global_proj.unsqueeze(1).expand(-1, seq_len, -1)
                    combined_features = torch.cat([combined_features, prosodic_global_expanded], dim=-1)
                
            except Exception as e:
                print(f"⚠️ Prosodic feature processing failed: {e}")
                # Continue without prosodic features
                pass
        
        # Multi-head attention for feature fusion
        attended_features, attention_weights = self.attention(
            combined_features, combined_features, combined_features
        )  # [batch, seq_len, total_features]
        
        # Global average pooling for window-level prediction
        window_features = attended_features.mean(dim=1)  # [batch, total_features]
        
        # Final classification
        boundary_logit = self.classifier(window_features)  # [batch, 1]
...
class IntelligentWindowLoss(nn.Module):
    ...
    Designed specifically for your task:
    - Balanced ~50/50 positive/negative windows
    - Binary classification (not dense prediction)
    - Clean, preprocessed windows (no easy negatives)
    ...
    - Optional boundary-aware weighting for harder examples
    """
    ...
    def forward(self, logits, targets, metadata=None):
        ...
        # Boundary-aware weighting (optional)
        if self.boundary_aware and metadata is not None:
            # Weight samples based on how close boundary is to window edge
            # Closer boundaries are harder to detect, so weight them more
            sample_weights = torch.ones_like(bce_loss)
            
            for i, meta in enumerate(metadata):
                if isinstance(meta, dict) and 'boundary_pos_in_window' in meta:
                    ...
                    if boundary_pos is not None:
                        ...
                        weight = 1.0 + (1.0 - min(distance_from_end / 1000, 1.0))
    ...
    def _update_class_statistics(self, targets):
        ...
        with torch.no_grad():
            pos_count = torch.sum(targets)
            neg_count = len(targets) - pos_count
            
            self.class_counts[1] += pos_count
    ...
    def _get_adaptive_weights(self, targets):
        ...
        # Calculate current class frequencies
        ...
        # Apply weights based on target labels
...
def main():
    ...
    config = {
        ...
        'pos_weight': 1.2,                   # COMPETITIVE: Slight positive bias
        'threshold': 0.4,                    # COMPETITIVE: Standard threshold
        'boundary_tolerance': 0.02,          # COMPETITIVE: ±20ms tolerance (competition standard)
        
        # COMPETITIVE TRAINING STRATEGY  
        'batch_size': 12,                    # Smaller batches for gradient accumulation
        'gradient_accumulation_steps': 2,    # Effective batch size: 12×2=24
        'num_epochs': 40,                    # Sufficient for competitive performance
        'learning_rate': 2e-4,               # COMPETITIVE: Higher LR for learning
        'weight_decay': 1e-5,                # Light regularization
        'use_cosine_scheduling': True,       # COMPETITIVE: Better than plateau
        
        # COMPETITIVE MODEL ARCHITECTURE
        'hidden_dim': 256,                   # COMPETITIVE: More capacity
        'dropout_rate': 0.2,                 # COMPETITIVE: Less dropout for capacity
        'use_prosodic': True,                # COMPETITIVE: Prosodic features like Shabber & Bansal
        'num_attention_heads': 8,            # COMPETITIVE: Multi-head attention
        
        # COMPETITIVE WINDOW GENERATION
        'window_duration': 0.5,              # COMPETITIVE: 0.5s windows for focused boundary detection
        'negative_exclusion_zone': 0.12,     # 120ms exclusion (6x separation)
    ...
    train_dataloader = DataLoader(train_subset, batch_size=config['batch_size'], shuffle=True, collate_fn=custom_collate_fn, num_workers=4, pin_memory=True, persistent_workers=True)
    ...
    try:
        # COMPETITIVE Multi-Scale Boundary Classifier
        model = CompetitiveMultiScaleBoundaryClassifier(
            ...
            use_prosodic=config['use_prosodic'],       # Prosodic features
            num_attention_heads=config['num_attention_heads']  # 8 heads
        )
    ...
    print("=" * 80)
    print(f"🚀 COMPETITIVE IMPROVEMENTS vs 85-88% F1 COMPETITORS:")
    print(f"   🎯 Multi-scale temporal convolutions (3x3 + 9x9 kernels)")
    print(f"   🎵 Prosodic features (energy, spectral, ZCR deltas)")
    print(f"   🧠 Multi-head attention (8 heads) for feature fusion")
    print(f"   📏 ±20ms tolerance (competition standard vs our ±80ms)")
    print(f"   🔥 Focal loss for hard example mining")
    ...
    print(f"\n🏆 COMPETITIVE pipeline completed at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("🎯 COMPETITIVE improvements: Multi-scale + prosodic + ±20ms tolerance!")
