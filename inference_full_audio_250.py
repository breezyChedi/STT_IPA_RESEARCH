"""
STANDALONE FULL AUDIO BOUNDARY DETECTION INFERENCE - 250ms MODEL
===================================================================

🏆 COMPETITIVE VERSION - Compatible with wav2seg_super_buffer_250ms.py
...
# =============================================================================
# COMPETITIVE CONFIGURATION - All parameters from wav2seg_super_buffer_250ms.py
# =============================================================================
...
COMPETITIVE_CONFIG = {
    ...
    'hidden_dim': 512,                    # COMPETITIVE: Double capacity to match checkpoint
    'prosodic_dim': 256,                  # COMPETITIVE: Prosodic processing dimension
    'dropout_rate': 0.2,                  # Standard dropout
    'use_prosodic': True,                 # COMPETITIVE: ALL DELTA-BASED prosodic features
    
    # WINDOW PROCESSING
    'window_duration': 0.25,              # COMPETITIVE: 250ms windows for focused boundary detection
    'sample_rate': 16000,                 # Standard sample rate
    'stride': 0.01,                       # 10ms stride for ±20ms precision
    'context_buffer': 0.02,               # 20ms boundary tolerance (competition standard)
    
    # COMPETITIVE TOLERANCES & THRESHOLDS
    'boundary_tolerance': 0.02,           # COMPETITIVE: ±20ms tolerance (competition standard)
    'threshold': 0.65,                    # COMPETITIVE: Standard decision threshold
    ...
    'prosodic_features': {
        ...
        'sequence_length': 13             # Default sequence length for 250ms windows
    },
...
try:
    from wav2seg_super_buffer_250ms import (
    ...
    print("✅ Successfully imported from wav2seg_super_buffer_250ms.py")
except ImportError as e:
    print(f"⚠️ Could not import from wav2seg_super_buffer_250ms.py: {e}")
    ...
    try:
        ...
        print("�� Make sure wav2seg_super_buffer_250ms.py or wav2seg_v4.py is in the same directory")
...
class CompetitiveMultiScaleBoundaryClassifier(nn.Module):
    ...
    def __init__(self, wav2vec2_model_name="facebook/wav2vec2-base", freeze_wav2vec2=True, 
                 hidden_dim=512, prosodic_dim=256, dropout_rate=0.2, use_prosodic=True):
        """
        ...
        Args:
            ...
            hidden_dim: Hidden dimension for classification head (512 for checkpoint)
            prosodic_dim: Dimension for prosodic processing (256 for checkpoint)
        ...
        if use_prosodic:
            ...
            self.prosodic_conv1 = nn.Conv1d(prosodic_input_dim, prosodic_dim, kernel_size=3, padding=1)
            self.prosodic_conv2 = nn.Conv1d(prosodic_dim, prosodic_dim, kernel_size=3, padding=1)
            
            # Total features after concatenation
            total_features = hidden_dim * 2 + prosodic_dim  # fine + coarse + prosodic
        else:
        ...
        self.boundary_detector = nn.Sequential(
            nn.Linear(hidden_dim, prosodic_dim),
            nn.ReLU(),
            nn.Dropout(dropout_rate),
            nn.Linear(prosodic_dim, 1)
        )
        ...
        if use_prosodic:
            self.prosodic_layer_norm = nn.LayerNorm(prosodic_dim)
    ...
    def forward(self, input_values, prosodic_features=None):
        """
        Forward pass with prosodic features.
        """
        ...
        if self.use_prosodic and prosodic_features is not None:
            ...
            prosodic_features = self.prosodic_dropout(prosodic_features)
            
            prosodic_features = F.relu(self.prosodic_conv1(prosodic_features))
            prosodic_features = self.prosodic_layer_norm(prosodic_features.transpose(1, 2))
            prosodic_features = prosodic_features.transpose(1, 2)
            
            prosodic_features = F.relu(self.prosodic_conv2(prosodic_features))
            feature_list.append(prosodic_features)
        ...
        # Get boundary scores and attention weights
        ...
        weighted_scores = timestep_scores * attention_weights
        
        # Final prediction
        prediction = torch.sum(weighted_scores, dim=1)  # [batch]
...
def predict_boundaries_competitive(model, audio, processor, device, 
    ...
    - ALL DELTA-BASED prosodic features (28 temporal sequences)
    - 250ms windows for focused boundary detection
    ...
    if verbose:
        ...
        print(f"   Threshold: {config['threshold']}, Boundary tolerance: ±{config['boundary_tolerance']*1000:.0f}ms")
    ...
    boundary_tolerance_samples = int(boundary_tolerance * sample_rate)
    ...
    with torch.no_grad():
        ...
        for window_start in range(0, len(audio) - window_samples + 1, stride_samples):
            ...
            # CRITICAL: Use training preprocessor for prosodic features
            ...
            if hasattr(model, 'use_prosodic') and model.use_prosodic:
                try:
                    ...
                    if seq_len <= 0:
                        seq_len = 13  # Default for 250ms windows
            ...
            # Calculate boundary position (center of window)
            boundary_pos = window_start + (window_samples // 2)
...
def load_model(model_path=None, device='auto', config=None):
    ...
    print(f"🤖 Loading 250ms COMPETITIVE model from {model_path}")
    print(f"🔧 Using device: {device}")
    print(f"🏆 Architecture: CompetitiveMultiScaleBoundaryClassifier")
    print(f"   Hidden dim: {config['hidden_dim']}")
    print(f"   Prosodic dim: {config['prosodic_dim']}")
    ...
    model = CompetitiveMultiScaleBoundaryClassifier(
        ...
        prosodic_dim=config['prosodic_dim'],  # COMPETITIVE: Prosodic dimension (256)
        dropout_rate=config['dropout_rate'],   # Standard dropout (0.2)
...
def load_timit_test_data(num_samples=15):
    ...
    try:
        ...
        print(f"�� Make sure wav2seg_super_buffer_250ms.py is in the same directory")
        return []
